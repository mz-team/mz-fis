...
function handleWeinreComment(obj, next) {
  var isWeinreMod = !!obj.options.weinre, 
      username = obj.options.weinre === true ? 'mz-fis' : obj.options.weinre;
  ...
  if (isWeinreMod) {
    makeLiveServer(function(error, server, port) {
      ...
      _.toArray(obj.modified).forEach(function(file) {
        ...
        content = content.replace(rWeinre, function(all, token) {
          if (token) {
            ...
            all = '<script type="text/javascript" charset="utf-8" src="http://' + hostname + ':' + port + '/target/target-script-min.js#' + username +'"></script>' + token;
          }
      ...
      openUrl('http://' + defaultHostname + ':' + weinrePort + '/client/#' + username);
